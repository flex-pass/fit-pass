...
.cinematic-footer-wrapper {
  ...
  /* Dynamic Variables using hardcoded colors to avoid shadcn token parsing errors */
...
.footer-aurora {
  background: radial-gradient(
    circle at 50% 50%, 
    color-mix(in srgb, #E11D48 15%, transparent) 0%, 
    color-mix(in srgb, #BE123C 15%, transparent) 40%, 
    transparent 70%
  );
}
...
const MarqueeItem = () => (
  <div className="flex items-center space-x-12 px-6">
    <span>One Pass</span> <span className="text-brand-primary/60">✦</span>
    <span>Every Gym</span> <span className="text-brand-secondary/60">✦</span>
    <span>Total Freedom</span> <span className="text-brand-primary/60">✦</span>
    <span>Contract-Free</span> <span className="text-brand-secondary/60">✦</span>
    <span>75+ Premium Partners</span> <span className="text-brand-primary/60">✦</span>
  </div>
);
...
export function CinematicFooter() {
  ...
  return (
    <>
      ...
      >
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-black text-zinc-50 cinematic-footer-wrapper">
          ...
          <div
            ref={giantTextRef}
            className="footer-giant-bg-text absolute -bottom-[5vh] left-1/2 -translate-x-1/2 whitespace-nowrap z-0 pointer-events-none select-none text-[#E11D48]/10 font-sans tracking-tighter"
          >
            FLEXPASS
          </div>

          <div className="absolute top-12 left-0 w-full overflow-hidden border-y border-zinc-800/50 bg-black/60 backdrop-blur-md py-4 z-10 -rotate-2 scale-110 shadow-2xl">
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-20 w-full max-w-5xl mx-auto">
            ...
            <div ref={linksRef} className="flex flex-col items-center gap-6 w-full">
              <div className="flex flex-wrap justify-center gap-4 w-full">
                <MagneticButton as="a" href="/register" className="footer-glass-pill px-10 py-5 rounded-full text-zinc-100 font-bold text-sm md:text-base flex items-center gap-3 group">
                  <svg className="w-6 h-6 text-zinc-500 group-hover:text-zinc-100 transition-colors" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6 w-full mt-2">
                <MagneticButton as="a" href="/privacy" className="footer-glass-pill px-6 py-3 rounded-full text-zinc-500 font-medium text-xs md:text-sm hover:text-zinc-100">
                  Privacy Policy
                </MagneticButton>
                <MagneticButton as="a" href="/terms" className="footer-glass-pill px-6 py-3 rounded-full text-zinc-500 font-medium text-xs md:text-sm hover:text-zinc-100">
                  Terms of Service
                </MagneticButton>
                <MagneticButton as="a" href="/support" className="footer-glass-pill px-6 py-3 rounded-full text-zinc-500 font-medium text-xs md:text-sm hover:text-zinc-100">
                  Support
                </MagneticButton>
              </div>
            </div>
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="footer-glass-pill px-6 py-3 rounded-full flex items-center gap-2 order-1 md:order-2 cursor-default border-zinc-800/50">
              <span className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">Crafted with</span>
              <span className="animate-footer-heartbeat text-sm md:text-base text-brand-primary">❤</span>
              <span className="text-zinc-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">for</span>
              <span className="text-zinc-100 font-black text-xs md:text-sm tracking-normal ml-1">Fitness</span>
            </div>

            <MagneticButton
              as="button"
              onClick={scrollToTop}
              className="w-12 h-12 rounded-full footer-glass-pill flex items-center justify-center text-zinc-500 hover:text-zinc-100 group order-3"
            >
